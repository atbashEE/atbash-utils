...
Based on the org.reflection code, but a very limited version which can scan for resources on classpath.
...
- JAR on the classpath are only taken into account when it contains a Manifest file (_/META-INF/MANIFEST.MF_)

Another useful method in some situation are the _geResourcePaths()_. They return the actual location (on the drive) of the resource.

If you notice that scanning the resources takes a lot of time, you can increase the performance by supplying an instance of an _ExecutorService_ so that classpath URL are scanned in a multi-threaded fashion.

You can evaluate if a multi-threaded apporach is required by looking at a log entry (info level) in the format of
